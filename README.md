...
קטלוג פידי RSS ישראלי מתוחזק: חדשות, טכנולוגיה, כלכלה ופיננסים, ספורט, לייףסטייל, יהדות, בלוגים וקהילות, ומקורות ישראליים באנגלית.

**53 פידים ב-8 קטגוריות.** כל פיד בקטלוג אומת מול האתר החי (תשובת HTTP תקינה, XML שמתפעל, ופריטים עדכניים) לפני שהוא נכנס, ונבדק מחדש אוטומטית כל שבוע.
...
- **גילוי פידים חדשים** - כל יום רביעי, Action שני מריץ את `scripts/discover_feeds.py`: סורק את האתרים ב-`scripts/discovery-sites.txt` (autodiscovery של תגיות link ונתיבים נפוצים), מאתר פידים חיים שעוד לא בקטלוג, ופותח issue עם המועמדים לאישור ידני. רוצים להוסיף אתר לסריקה? PR ל-`discovery-sites.txt`.

## תרומה

1. מוסיפים או מעדכנים פיד ב-`feeds.json` (הקובץ הוא מקור האמת).
2. מריצים `python scripts/build_opml.py` כדי לייצר מחדש את קבצי ה-OPML.
